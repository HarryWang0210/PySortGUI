...
def find_threshold_crossings(x, threshold=None, deadtime=0, multiplier=THRESHOLD_MULTIPLIER):
    '''
...
def find_valleys_after_threshold_crossings(x, threshold=None, deadtime=48,
                                           deadtime_alg='keep_max', multiplier=THRESHOLD_MULTIPLIER):
    '''
...
def get_crossings(x, alg='Valley-Peak', threshold=None, deadtime=25,
                  dual_threshold=False, threshold_sign=None, extract_after=0, multiplier=None):
    '''
...
def extract_waveforms(x, wav_length=36, alg='Crossings', threshold=None,
                      deadtime=42, dual_threshold=False, amplitude_rejection=False, log_level='INFO',
                      ts_type='Spikes', threshold_sign=None, extract_after=0, multiplier=None):
    '''
    ...
    crossings, Threshold = get_crossings(
        x, alg, threshold, deadtime, dual_threshold=dual_threshold,
        threshold_sign=threshold_sign, extract_after=extract_after, multiplier=multiplier)

    points_before = wav_length / 4
    if alg == 'Valley-Peak':
        points_before = wav_length * 3 / 8
    ...
    Header['SamplingFreq'] = x.Header.get('SamplingFreq', 1)
    Header['Threshold'] = Threshold
    Header['NumRecords'] = crossings.size
    pID = x.Header.get('ID', None)

    if pID <= 128:
        type_ = 'Spikes'
        Header['H5Location'] = '/Spikes/spike{:03d}'.format(pID)
    elif pID > 128:
        type_ = 'Events'
        Header['H5Location'] = '/Events/event{:03d}_offline'.format(pID)
    else:
        type_ = 'Unknown'
        Header['H5Location'] = '/Unknown/unkwn{:03d}'.format(pID)
