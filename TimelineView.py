...
class GLWidget(QOpenGLWidget):
    ...
    def paintGL(self):
        ...
        # glScale(1 * self.scale_x, 1.0, 1.0)

        # line_strip_data = []
        # for i, y in enumerate(self.data[self.offset:self.offset+10]):
        #     x = -1.0 + i * 0.2
        #     line_strip_data.extend([x, y])
        # self.draw_line_strip(line_strip_data)

        # glPopMatrix()

        # color_location = self.shader_program.uniformLocation('color')
        # self.shader_program.setUniformValue(
        #     color_location, 1.0, 1.0, 1.0)  # line color
